...
const FeaturesIcon = () => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2 L14.09 8.09 L20.18 10 L14.09 11.91 L12 18 L9.91 11.91 L3.82 10 L9.91 8.09 L12 2 z"/></svg>;
        ...
        <>
            ...
            <nav className="hidden md:flex fixed top-0 left-0 right-0 z-50 items-center justify-between h-20 px-8 bg-zinc-900/80 backdrop-blur-sm border-b border-zinc-800">
                ...
                <ul className="flex items-center space-x-8">
                    {navLinks.map(link => (
                        <li key={link.href}>
                            <a href={link.href} className="text-zinc-300 hover:text-red-400 transition-colors duration-300 font-medium">
                                {link.label}
                            </a>
                        </li>
                    ))}
            ...
            <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 px-5 pb-5 pt-2">
                <div className="bg-zinc-800/90 backdrop-blur-sm h-20 rounded-full flex justify-around items-center border border-zinc-700 shadow-2xl shadow-red-500/10">
                    {mobileNavLinks.map(link => (
                        <a key={link.href} href={link.href} className="flex flex-col items-center text-zinc-400 hover:text-red-400 transition-colors duration-300 space-y-1">
                            <div className="w-7 h-7">{link.icon}</div>
                            <span className="text-xs font-medium">{link.label}</span>
                        </a>
                    ))}
                     <button onClick={onRegisterClick} className="flex flex-col items-center text-zinc-400 hover:text-red-400 transition-colors duration-300 space-y-1">
